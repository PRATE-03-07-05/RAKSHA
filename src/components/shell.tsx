...
export function LogoMark({ size = 34 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" aria-hidden="true">
      <path
        d="M16 2l11 4v9c0 7.5-4.7 12.6-11 15C9.7 27.6 5 22.5 5 15V6l11-4z"
        fill="#0f3d33"
      />
      <path
        d="M9 16h4l2-5 3 9 2-4h3"
        stroke="#4ade80"
        strokeWidth="2.2"
        fill="none"
        strokeLinecap="round"
...
/**
 * Shared navigation for ASHA and ANM field-health-worker roles.
 *
 * IMPORTANT:
 * Keep this outside NAV so ANM can reuse it without creating a
 * circular initialization dependency on NAV.
 */
const FIELD_WORKER_NAV: NavItem[] = [
  {
    to: "/app/dashboard",
    key: "dashboard",
  ...
  },
  {
    to: "/app/referrals",
    key: "referrals",
    icon: <Signpost className="h-[18px] w-[18px]" />,
  },
  {
    to: "/app/followups",
    key: "followups",
...
/**
 * Doctor navigation shared by PHC Doctor, CHC Doctor and Specialist.
 */
function DOCTOR_NAV(): NavItem[] {
  return [
    ...
    },
    {
      to: "/app/queue",
      key: "patientQueue",
      icon: <ListOrdered className="h-[18px] w-[18px]" />,
    },
    {
      to: "/app/patients",
      key: "patients",
 ...
 *
 * ASHA and ANM both use FIELD_WORKER_NAV.
 * This avoids the previous NAV -> NAV_ANM() -> NAV.ASHA circular
...
const NAV: Record<Role, NavItem[]> = {
  ...
  ASHA: FIELD_WORKER_NAV,

  ANM: FIELD_WORKER_NAV,
    ...
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-3 px-5 pb-6 pt-6">
        ...
        <div>
          ...
          <p className="mt-1 text-[10px] font-medium uppercase tracking-[0.16em] text-brand-300">
            Rural Health Net
          </p>
        </div>
      </div>
      ...
      >
        {nav.map((item) => (
          ...
          >
            <span className="text-brand-300 group-[.active]:text-emerald-300">
              {item.icon}
            </span>
      ...
      <div className="border-t border-white/10 p-4">
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            aria-label={t("logout")}
            title={t("logout")}
            className="rounded-lg p-2 text-brand-300 transition hover:bg-white/10 hover:text-white"
          >
            <LogOut className="h-4.5 w-4.5" />
          </button>
        </div>

        <p className="mt-3 text-center text-[10px] text-brand-400">
          Prototype build
        </p>
      </div>
    </div>
...
function NotificationsDrawer({
  ...
  const { data, loading } = useApi(
    () => api.listNotifications(user as never),
    [user?.id]
  );
  const { t } = useI18n();
      ...
      <aside className="anim-fade-in absolute right-0 top-0 h-full w-[min(94vw,400px)] overflow-y-auto border-l border-brand-900/10 bg-white shadow-pop">
        <header className="sticky top-0 flex items-center justify-between border-b border-brand-900/10 bg-white px-5 py-4">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => {
                void api.markNotificationsRead(user as never);
              }}
              className="inline-flex items-center gap-1.5 rounded-lg px-2 py-1.5 text-xs font-semibold text-brand-700 hover:bg-brand-50"
            >
        ...
        <div className="space-y-2 p-4">
          ...
          {(data ?? []).map((n) => (
            <button
              ...
              className={cx(
                "w-full rounded-xl border p-3.5 text-left transition hover:border-brand-300 hover:bg-brand-50/60",
                n.read
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    {!n.read && (
                      <span className="h-2 w-2 shrink-0 rounded-full bg-clay-500" />
                    )}
                  </div>
...
export function AppShell() {
  ...
  const connTone = offline
    ? "bg-rose-50 text-rose-800 border-rose-200"
    : syncing
      ? "bg-amber-50 text-amber-800 border-amber-200"
      : "bg-emerald-50 text-emerald-800 border-emerald-200";

  const connLabel = offline
    ? t("offline")
    : syncing
      ? t("syncing")
      : t("online");
        ...
        <div className="no-print fixed inset-0 z-[60] lg:hidden">
          ...
          <aside className="anim-fade-in absolute inset-y-0 left-0 w-[262px] bg-brand-950 shadow-pop">
            <button
              onClick={() => setDrawer(false)}
              aria-label={t("close")}
              className="absolute right-3 top-4 rounded-lg p-2 text-brand-300 hover:bg-white/10"
            >
              <X className="h-5 w-5" />
            </button>
          ...
          <div className="flex items-center justify-between gap-3 px-4 py-3 sm:px-6">
            <div className="flex items-center gap-3">
              <button
                ...
                className="rounded-lg border border-brand-900/10 bg-white p-2 text-brand-800 shadow-sm lg:hidden"
              >
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                title={
                  simOffline
                    ? "Reconnect (demo control)"
                    : "Simulate offline (demo control)"
                }
                className={cx(
              ...
              <div
                className="hidden items-center overflow-hidden rounded-full border border-brand-900/10 bg-white md:flex"
                role="group"
              ...
              <button
                ...
                aria-label={t("notifications")}
                className="relative rounded-lg border border-brand-900/10 bg-white p-2 text-brand-800 shadow-sm transition hover:border-brand-300"
              >
                <Bell className="h-[18px] w-[18px]" />

                {unread > 0 && (
                  <span className="absolute -right-1 -top-1 flex h-4.5 min-w-4.5 items-center justify-center rounded-full bg-clay-500 px-1 text-[10px] font-bold text-white">
                    {unread}
                  </span>
                )}
              </button>
